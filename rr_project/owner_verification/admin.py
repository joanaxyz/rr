...
class OwnerVerificationRequestAdmin(admin.ModelAdmin):
    ...
    search_fields = ("user__username", "gov_full_name", "id_number", "business_address")
    ...
    def cancel_requests(self, request, queryset):
        for obj in queryset:
            if obj.state != "CANCELLED":
                ...
                user = obj.user
                send_mail(
                    subject="Owner Verification Cancelled",
                    message=f"Hi {user.username},\n\nYour owner verification request has been CANCELLED by admin.",
                    from_email=settings.DEFAULT_FROM_EMAIL,
                    recipient_list=[user.email],
                    fail_silently=True,
                )
