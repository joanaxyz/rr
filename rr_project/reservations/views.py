...
def reservation_management_view(request):
    """Display, cancel, restore, or edit reservations"""
    ...
    if request.method == 'POST':
        ...
        # Restore reservation
            ...
            try:
                ...
                # restore the previous status or set default
                reservation.status = reservation.previous_status or 'PENDING'
                reservation.previous_status = None
                reservation.cancellation_reason = None
                reservation.save()

                messages.success(request, f"Reservation at {restaurant_name} has been restored.")
            except Exception as e:
                print(e)
